import { post, deletes, headersAddBearerToken } from "../request.js";
...
export default class Auth extends Api {
  ...
  login(username, password) {
    return post(`${this.endpoint}/login/`, { username, password }).then(
      body => {
        return {
          token: body.token,
          user: body.user,
  ...
  generateToken() {
    ...
    return post(`${this.endpoint}/login/`, null, headers).then(body => {
      return {
        token: body.token,
        user: body.user,
      };
